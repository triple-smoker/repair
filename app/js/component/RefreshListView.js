...
export default class RefreshListView extends Component {
  ...
  render() {
    const { dataSource, renderRow, isRefreshing, renderSeparator } = this.props;
   ...
   */
  _hasMore() {
    return this.props.cachedResults.items.length < this.props.cachedResults.total;
  }
   ...
   */
  _renderFooter() {
    //console.log('items.length = '+this.props.cachedResults.items.length + ', items.total = ' + this.props.cachedResults.total);
    if (!this._hasMore() && this.props.cachedResults.total !== 0) {
      return (
        <View style={styles.loadingMore2}>
          <Text style={styles.loadingText}>没有更多内容了</Text>
        </View>
      )
    } else if (!this.props.isRefreshing && !this.props.isLoadingTail && this.props.cachedResults.items.length === 0 && this.props.cachedResults.total === 0) {
      return (
